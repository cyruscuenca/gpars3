...
#pragma DebuggerWindows("debugStream"); //Opens debug stream
...
// if file is not found/cannot open
if (fd < 0){
...
// Calculate the distance (delta) from the current position to the new one and update the current position
float calcDeltaDistance(float &currentPosition, float newPosition){
